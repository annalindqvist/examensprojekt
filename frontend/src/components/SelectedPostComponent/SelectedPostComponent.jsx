...
const SelectedPostComponent = ({ post }) => {
  ...
  const handleLike = async () => {
    ...
    if (token) {
      ...
      const res = await fetch('http://localhost:8080feed/like/' + postId,  {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    if (token && postedByUser) {
      ...
      const res = await fetch('http://localhost:8080feed/delete/' + postId,  {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleComment = async (e) => {
    ...
    if (token) {
        ...
        const res = await fetch(`http://localhost:8080feed/comment/${postId}`, {
        method: 'POST',
        headers: {
